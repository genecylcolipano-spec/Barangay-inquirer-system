...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const readMoreButtons = document.querySelectorAll('[data-toggle="collapse"]');
        ...
        readMoreButtons.forEach(function(button) {
            button.addEventListener('click', function(e) {
                ...
                const previewText = this.previousElementSibling; // The <p> tag right before button
                
                if (previewText && previewText.classList.contains('preview-text')) {
                    // Immediately hide preview text
                    previewText.style.display = 'none';
                    
                    // Show the collapse
                    $(collapseElement).collapse('show');
                } else {
                    // Fallback: just trigger collapse
                    $(collapseElement).collapse('toggle');
                }
            });
        });
        
        // Listen for when collapse is hidden to show preview again
        $(document).on('hidden.collapse', '[class*="collapse"]', function() {
            // Find the preview text in the same card
            const card = $(this).closest('.card');
            const previewText = card.find('.preview-text');
